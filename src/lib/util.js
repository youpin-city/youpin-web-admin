...
utility.site_url = function (pathname, basepath, query, hash) {
  pathname = pathname || '/';
  if (typeof basepath === 'object' && basepath !== null) {
    hash = query;
    query = basepath;
    basepath = null;
  }
 ...
 * @return {void}
 */
utility.page_info = function (req, path = '', title, options) {
  options = options || {};
  const logical_path = req.url.split('?')[0];
...
utility.promise_while = promiseFor;
